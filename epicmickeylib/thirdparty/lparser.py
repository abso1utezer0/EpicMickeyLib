# https://github.com/CPunch/LuaDecompy/blob/main/lparser.py
'''
    lparser.py
...
from epicmickeylib.thirdparty.lundump import Chunk, Constant, Instruction, Opcodes, whichRK, readRKasK
...
class LuaDecomp:
    ...
    def __getReg(self, indx: int) -> str:
        ...
        try:
            ...
            # if it's not a local, make it one
    ...
    def parseInstr(self):
        ...
        elif instr.opcode == Opcodes.LOADNIL:
            # load nil into registers
            for i in range(instr.A, instr.B):
                self.__setReg(i, "nil")
